...
def ask_gpt_via_openai(messages):
    """Резервный вызов через OpenAI если все Gemini ключи исчерпаны."""
    ...
    for key in keys_to_try:
        try:
            ...
            if "429" in error_text or "quota" in error_text.lower() or "rate" in error_text.lower():
                print("OpenAI ключ исчерпан, пробуем следующий...")
                continue
            raise
...
def ask_gpt(messages, attempt=0):
    """Бесплатная модель: сначала Gemini, при исчерпании — OpenAI fallback."""
